...
import { eventSource, event_types, saveSettingsDebounced, generateQuietPrompt } from '../../../../script.js';
...
<div id="mrag-panel" class="mrag-panel">
  ...
  <details class="mrag-section mrag-collapsible">
    ...
    <div class="mrag-body">
      ...
      <div id="mrag-llm-opts" class="${s.llmClassifier ? '' : 'mrag-hidden'}">
        <p class="mrag-hint">Оставь пустым — будет использован встроенный генератор SillyTavern. Или укажи внешний OpenAI-совместимый API.</p>
        <label class="mrag-label">LLM Endpoint (необязательно)</label>
        <input type="text" id="mrag-llm-endpoint" class="mrag-input" placeholder="https://api.openai.com" value="${s.llmEndpoint || ''}">
        <label class="mrag-label">API Key (необязательно)</label>
        <input type="password" id="mrag-llm-key" class="mrag-input" placeholder="sk-..." value="${s.llmApiKey || ''}">
        <label class="mrag-label">Модель (необязательно)</label>
        <input type="text" id="mrag-llm-model" class="mrag-input" placeholder="gpt-4o-mini" value="${s.llmModel || ''}">
      </div>
...
function bindEvents() {
  ...
  $('mrag-llm')?.addEventListener('change',        e => {
    save({ llmClassifier: e.target.checked });
    const opts = document.getElementById('mrag-llm-opts');
    if (opts) opts.classList.toggle('mrag-hidden', !e.target.checked);
  });
  $('mrag-llm-endpoint')?.addEventListener('input',  e => save({ llmEndpoint: e.target.value }));
  $('mrag-llm-key')?.addEventListener('input',       e => save({ llmApiKey: e.target.value }));
  $('mrag-llm-model')?.addEventListener('input',     e => save({ llmModel: e.target.value }));
...
class MemoryExtractor {
  ...
  async _extractLLM(messages) {
    ...
    // ── Method 1: External OpenAI-compatible API (if endpoint + key configured) ──
    const s = cfg();
    const extEndpoint = (s.llmEndpoint || '').trim().replace(/\/+$/, '').replace(/\/v1$/, '');
    const extKey      = (s.llmApiKey  || '').trim();
    const extModel    = (s.llmModel   || '').trim() || 'gpt-4o-mini';

    if (extEndpoint && extKey) {
      try {
        _pushLog('INFO', 'LLM extract: внешний API ' + extEndpoint);
        const res = await fetch(extEndpoint + '/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + extKey },
          body: JSON.stringify({
            model: extModel,
            messages: [
              { role: 'system', content: 'You extract memories from chat logs. Respond ONLY with a JSON array, no markdown, no commentary.' },
        ...
        _pushLog('INFO', 'Внешний API → HTTP ' + res.status);
        if (res.ok) {
          const d = await res.json();
          raw = d?.choices?.[0]?.message?.content ?? '';
          if (raw) _pushLog('INFO', 'Внешний API OK, получено ' + raw.length + ' символов');
        } else {
          const errText = await res.text().catch(() => '');
          _pushLog('WARN', 'Внешний API ошибка ' + res.status + ': ' + errText.slice(0, 120));
        }
      } catch (e) {
        _pushLog('WARN', 'Внешний API исключение: ' + (e?.message || String(e)));
      }
    }

    // ── Method 2: SillyTavern built-in generateQuietPrompt (imported from script.js) ──
    if (!raw) {
      try {
        if (typeof generateQuietPrompt !== 'function') throw new Error('generateQuietPrompt недоступна');
        ...
        if (raw) _pushLog('INFO', 'generateQuietPrompt OK, получено ' + raw.length + ' символов');
      } catch (e) {
        _pushLog('WARN', 'generateQuietPrompt: ' + (e?.message || String(e)));
      }
    }

    if (!raw) throw new Error('Нет ответа от LLM — все методы не сработали');

    // ── Parse JSON ──
    const clean = raw.replace(/```json|```/g, '').trim();
    const match  = clean.match(/\[[\s\S]*?\]/);
    if (!match) {
      _pushLog('WARN', 'LLM не вернул JSON-массив. Ответ: ' + clean.slice(0, 120));
      return [];
    }
    try {
