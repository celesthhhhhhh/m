  ...
  async extract(messages) {
    ...
    if (s.llmClassifier) {
      try {
        ...
        // LLM returned empty array — fall through to heuristic as backup
        if (result && result.length > 0) return result;
        logWarn('LLM extraction returned empty, falling back to heuristic');
      } catch (e) {
        logWarn('LLM extraction failed, falling back to heuristic:', e?.message ?? String(e));
      }
    }
...
${conversation}`;
    ...
    // SillyTavern's correct generate endpoint
    let raw = '';
    try {
      const ctx = SillyTavern.getContext();
      // Use the built-in generate function with a temporary override
      raw = await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('LLM timeout')), 30000);
        ctx.generateQuietPrompt(prompt, false, false).then(r => {
      ...
      // generateQuietPrompt not available — try REST fallback
      logWarn('generateQuietPrompt unavailable, trying REST:', e?.message);
      const endpoints = [
      ...
      for (const ep of endpoints) {
        try {
          ...
          if (res.ok) { const d = await res.json(); raw = d.results?.[0]?.text ?? d.choices?.[0]?.text ?? d.content ?? ''; break; }
          lastErr = 'HTTP ' + res.status;
        } catch (fe) { lastErr = fe?.message; }
      }
      if (!raw) throw new Error('All LLM endpoints failed: ' + lastErr);
    }
